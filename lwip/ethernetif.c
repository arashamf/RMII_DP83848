...
static void low_level_init(struct netif *netif)
{
  ...
  #else
  vSemaphoreCreateBinary(RxPktSemaphore);
  #endif
  ...
  #else
  vSemaphoreCreateBinary(TxPktSemaphore);
  #endif
...
xTaskCreate((TaskFunction_t )ethernetif_input, (const char*)"EthIf", INTERFACE_THREAD_STACK_SIZE, netif, (UBaseType_t)0, (TaskHandle_t*)&os_thread_def_EthIf);
#endif
  ...
  do
  {
    ...
    else
    {
      ...
      if(HAL_ETH_GetError(&heth) & HAL_ETH_ERROR_BUSY)
      {
        ...
        #else
        //BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        xSemaphoreTake (TxPktSemaphore, ( portTickType ) ETHIF_TX_TIMEOUT);
        #endif
...
static void ethernetif_input(void const * argument)
{
  ...
  for( ;; )
  {
    ...
    #else
    if (xSemaphoreTake (RxPktSemaphore, ( portTickType ) 100) == pdTRUE )
    #endif
    {
      do
      {
